...
coremark_bin = ./bin/custom-output/benchmark/coremark/coremark.bin
...
while getopts 'craxyt:w:' OPT; do
  case $OPT in
    ...
    x)
      ./build/emu -i ${coremark_bin} ;;
    y)
      ./build/emu -i ${coremark_bin} -b 0 --dump-wave --wave-path=./build/coremark.fst;;
    t)
      example="$OPTARG"
