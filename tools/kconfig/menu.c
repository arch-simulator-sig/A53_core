...
void menu_set_type(int type)
{
    ...
    menu_warn(current_entry, "ignoring type redefinition of '%s' from '%s' to '%s'", sym->name ? sym->name : "<choice>",
...
{
    if (modules_sym)
        zconf_error("symbol '%s' redefines option 'modules' already defined by symbol '%s'", current_entry->sym->name,
                    modules_sym->name);
        ...
        for (menu = parent->list; menu; menu = menu->next)
        {
            ...
            for (; prop; prop = prop->next)
            {
                ...
                if (prop->type == P_SELECT)
                {
                    ...
                    es->rev_dep.expr =
                        expr_alloc_or(es->rev_dep.expr, expr_alloc_and(expr_alloc_symbol(menu->sym), expr_copy(dep)));
                }
                else if (prop->type == P_IMPLY)
                {
                    struct symbol *es = prop_get_symbol(prop);
                    es->implied.expr =
                        expr_alloc_or(es->implied.expr, expr_alloc_and(expr_alloc_symbol(menu->sym), expr_copy(dep)));
                }
            }
        }
...
static void get_prompt_str(struct gstr *r, struct property *prop, struct list_head *head)
{
    ...
    if (i > 0)
    {
        ...
        for (j = 4; --i >= 0; j += 2)
        {
            ...
            if (menu->sym)
            {
                str_printf(r, " (%s [=%s])", menu->sym->name ? menu->sym->name : "<choice>",
